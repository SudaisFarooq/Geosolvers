...
# # ----------------------------
# rf_model_path = "C:/Users/Lenovo/New folder/random_forest_model .pkl"
# with open(rf_model_path, 'rb') as f:
#     rf_model = pickle.load(f)
# print(f"Loaded rf_model type: {type(rf_model)}")
...
#     model_path = f'lstm_model_{v}.h5'
#     scaler_path = f'scaler_{v}.pkl'
#     try:
#         model = tf.keras.models.load_model(model_path, compile=False)
#         lstm_models[v] = model
#         with open(scaler_path, 'rb') as f:
#             scalers[v] = pickle.load(f)
#         print(f"Loaded model and scaler for {v}")
...
#     "Jaspur": (29.29, 78.82),
#     "dineshpur": (28.97, 79.42),
#     "Haldwani": (29.22, 79.52),
#     "Nainital": (29.38, 79.45),
#     "sitarganj": (28.93, 79.70),
#     "nanakmatta": (28.99, 79.88),
#     "Bazpur": (29.15, 79.13),
#     "pantnagar": (28.98, 79.41),
#     "Pilibhit": (28.63, 79.80)
# }

# # ----------------------------
# # 4. Village elevations (meters)
# # ----------------------------
# village_elevations = {
...
# # ----------------------------
# # 5. Fetch recent rainfall data
# # ----------------------------
# def fetch_recent_data(lat, lon, days=30):
...
#     except Exception as e:
#         raise ValueError(f"NASA POWER fetch failed: {e}")

#     if not data:
#         raise ValueError(f"No rainfall data found for coordinates: {lat}, {lon}")
...
#     df['rainfall'] = df['rainfall'].apply(lambda x: np.nan if x < 0 or x > 1000 else x)
#     df['rainfall'] = df['rainfall'].ffill().fillna(0)
#     return df

# # ----------------------------
# # 6. Predict rainfall using LSTM
# # ----------------------------
...
#         pred_reshaped = np.array(pred_scaled).reshape(1, 1, 1)
...
# # ----------------------------
# # 7. Predict flood and return elevation + rainfall
# # ----------------------------
# def predict_flood(village):
...
#    # Fetch at least 60 days of historical rainfall
#     df = fetch_recent_data(lat, lon, days=max(100, time_steps + 7))

#     scaler = scalers[village]
#     rain_scaled = scaler.transform(df[['rainfall']].values)

#     if len(rain_scaled) < time_steps:
#         raise ValueError(f"Not enough historical data to form LSTM sequence for {village}")
